...
@click.option('--min_samples_leaf', default=2, required=True)
@click.option('--n_jobs', default=-1, required=True)
@click.option('--x', '-x', required=True, multiple=True)
@click.option('--y', '-y', required=True)
def train(n_estimators, max_depth, min_samples_leaf, n_jobs, x, y):
    startTime = time.time()
