...
@click.option('--max_depth', default=5, required=True)
@click.option('--min_samples_leaf', default=3, required=True)
@click.option('--max_features', default='sqrt', required=True)
@click.option('--x', '-x', required=True, multiple=True)
@click.option('--y', '-y', required=True)
def train(n_estimators, max_depth, min_samples_leaf, max_features, x, y):
    startTime = time.time()
